...
class BackgroundSolution:
    ...
    @classmethod
    def load(cls, path: str | Path) -> "BackgroundSolution":
        """Load a background saved by :meth:`save`."""
        ...
        with np.load(Path(path), allow_pickle=False) as data:
            return cls(
                ...
                dpsi=np.asarray(data["dpsi"], dtype=float),
                noether_charge=float(data["noether_charge"]),
                r99=float(data["r99"]),
                compactness99=float(data["compactness99"]),
                r999=float(data["r999"]),
                compactness999=float(data["compactness999"]),
                max_ode_residual=float(data["max_ode_residual"]),
                tail_residual=float(data["tail_residual"]),
                solver_status=int(data["solver_status"]),
                solver_message=str(data["solver_message"]),
            )
