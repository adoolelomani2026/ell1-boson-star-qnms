...
class RadialBackground:
    ...
    def __init__(
        ...
        representation: str = "hermite",
        geometry_representation: str = "pchip",
    ):
        ...
        self.geometry_representation = geometry_representation
        ...
        if geometry_representation == "pchip":
            self._mass_spline = PchipInterpolator(
                solution.r, solution.mass, extrapolate=False
            )
            self._alpha_spline = PchipInterpolator(
                solution.r, solution.alpha, extrapolate=False
            )
        elif geometry_representation == "hermite":
            background_fields = np.vstack(
                (solution.mass, solution.alpha, solution.psi, solution.dpsi)
            )
            background_derivatives = _rhs(
                solution.r, background_fields, solution.omega, solution.ell
            )
            self._mass_spline = CubicHermiteSpline(
                solution.r,
                solution.mass,
                background_derivatives[0],
                extrapolate=False,
            )
            self._alpha_spline = CubicHermiteSpline(
                solution.r,
                solution.alpha,
                background_derivatives[1],
                extrapolate=False,
            )
        else:
            raise ValueError("geometry_representation must be 'pchip' or 'hermite'")
        regular_du = (
    ...
    def lapse_second_derivative(self, radius: float | np.ndarray) -> np.ndarray:
        """Return alpha'' from the same interpolant used for alpha.

        The axial Einstein operator contains alpha''.  Keeping this derivative
        tied to the lapse interpolant avoids mixing an independently sampled
        numerical derivative into the coupled nonradial system.
        """

        radii = np.asarray(radius, dtype=float)
        if np.any(radii < self.r_min) or np.any(radii > self.r_max):
            raise ValueError("radius outside background domain")
        return np.asarray(self._alpha_spline.derivative(2)(radii))

    def equilibrium_lapse_second_derivative(
        self, radius: float | np.ndarray
    ) -> np.ndarray:
        """Return ``alpha''`` reconstructed from the first-order EKG system.

        This derivative uses the same mass, lapse, and regular scalar
        representation as :meth:`arrays`, but differentiates the equilibrium
        field equation analytically.  It is therefore suitable for dependent
        Einstein-equation checks, where taking a second derivative of an
        interpolating polynomial can otherwise dominate the residual.
        """
        ...
        fields = np.vstack((mass, alpha, psi, dpsi))
        mass_prime, alpha_prime, _, _ = _rhs(
            radii, fields, self.omega, self.ell
        )
        inverse_gamma2 = 1.0 - 2.0 * mass / radii
        gamma2 = gamma**2
        angular = self.ell * (self.ell + 1.0) / radii**2
        scalar_potential = self.omega**2 / alpha**2 - 1.0 - angular
        bracket = inverse_gamma2 * dpsi**2 + scalar_potential * psi**2
        inverse_gamma2_prime = -2.0 * mass_prime / radii + 2.0 * mass / radii**2
        scalar_potential_prime = (
            -2.0 * self.omega**2 * alpha_prime / alpha**3
            + 2.0 * self.ell * (self.ell + 1.0) / radii**3
        )
        bracket_prime = (
            inverse_gamma2_prime * dpsi**2
            + 2.0 * inverse_gamma2 * dpsi * ddpsi
            + scalar_potential_prime * psi**2
            + 2.0 * scalar_potential * psi * dpsi
        )
        source = mass / radii**2 + 0.5 * self.kappa * radii * bracket
        source_prime = (
            mass_prime / radii**2
            - 2.0 * mass / radii**3
            + 0.5 * self.kappa * (bracket + radii * bracket_prime)
        )
        gamma2_prime = 2.0 * gamma2 * log_gamma_prime
        log_alpha_prime_derivative = gamma2_prime * source + gamma2 * source_prime
        result = alpha * (
            log_alpha_prime**2 + log_alpha_prime_derivative
        )
